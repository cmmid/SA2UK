...
.args <- if (interactive()) sprintf(c(
  "../covid-policy-tracker", "%s/outputs/ox_si_timing.csv"
), .debug) else commandArgs(trailingOnly = TRUE)

rt <- file.path(path.expand(.args[1]), "data", "timeseries")

csvs <- grep("^index_", list.files(
  rt,
  pattern = "^\\w\\d_.*\\.csv$"
), invert = TRUE, value = TRUE)

readoxcsv <- function(fn, from) {
  #' read file, dropping country name column
  dt <- fread(file.path(from, fn), header = TRUE)[,.SD,.SDcols = -c(1,3)]
  melt(dt, id.vars = "country_code")[country_code != "", .(
    value = suppressWarnings(as.integer(value)),
    indicator = sub("-","_",gsub("_","",sub("_", "-", gsub(".csv", "", fn, fixed = TRUE))))
  ), keyby=.(
    iso3 = country_code,
...
all.indicators <- rbindlist(lapply(csvs, readoxcsv, from = rt))
mlt.indicators <- dcast(all.indicators, iso3 + date ~ indicator)
# all of the NAs up to first measure assumed NA?

mlt.indicators[c1_schoolclosing == 0, c1_flag := 0]
mlt.indicators[c2_workplaceclosing == 0, c2_flag := 0]
mlt.indicators[c3_cancelpublicevents == 0, c3_flag := 0]
mlt.indicators[c4_restrictionsongatherings == 0, c4_flag := 0]
mlt.indicators[c5_closepublictransport == 0, c5_flag := 0]
mlt.indicators[c6_stayathomerequirements == 0, c6_flag := 0]
mlt.indicators[c7_movementrestrictions == 0, c7_flag := 0]
mlt.indicators[h1_publicinformationcampaigns == 0, h1_flag := 0]
mlt.indicators[h6_facialcoverings == 0, h6_flag := 0]

w = 1/c(c1=3, c2=3, c3=3, c4=4, c5=2, c6=3, c7=2, 
  h1=2,
  h6=4
)
